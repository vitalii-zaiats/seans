...
Future<void> main() async {
  ...
  print('  ${first.name} | ${first.yearLabel} | imdb ${first.imdbMark} | ${first.time}');
  ...
  print('  movie: ${movie.allGenres.length} genres, ${movie.totalCount} titles');
  print('  ranges: ${movie.years.where((y) => y.isRange).map((y) => y.slug).join(', ')}');

  final page = await api.catalog(type: ContentType.serial, page: 1);
  print('catalog: ${page.length} of ${page.meta.total}, next ${page.meta.nextPage}');

  final hits = await api.search('мерц');
  print('search: ${hits.map((h) => h.card.slug).take(3).join(', ')}');
  print('  spans: ${hits.first.nameSpans().map((s) => s.toString()).join('|')}');

  final details = await api.content(first.slug);
  print('details: ${details.name} | seasons ${details.seasons.length} | cast ${details.cast.length}');
  final season = details.firstPlayableSeason;
  print('  playable: ${details.isPlayable} | season ${season?.number} '
  ...
  print('tv: ${channels.items.length} channels, ${channels.categories.length} categories');
