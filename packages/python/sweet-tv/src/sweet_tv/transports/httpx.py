...
It suits the seam exactly: it does not raise on a 4xx or 5xx (which the client
must see for itself) and it does raise on socket, DNS, TLS and timeout failures
(which the client wraps in `NetworkError`).
"""
...
def _as_response(response: httpx.Response) -> Response:
    # `.content`, not `.text`: this API omits the charset on
    # `application/json`, so letting httpx guess would mangle Ukrainian titles.
    return Response(
